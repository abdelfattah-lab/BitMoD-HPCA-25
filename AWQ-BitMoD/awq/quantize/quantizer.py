...
from ..utils.module import set_op_by_name, get_op_name
...
def pseudo_quantize_tensor(
    w, n_bit=8, module_name=None, zero_point=True, q_group_size=-1, inplace=False, get_scale_zp=False, wq_dtype=None, layer_dtype=None
):
    org_w_shape = w.shape
    if q_group_size > 0:
    ...
    # Modified by Yuzong Chen (yc2367@cornell.edu)
    if (wq_dtype is None) or ('int' in wq_dtype):
        ...
        if (wq_dtype in ["fp16", "fp32"]):
            pass
        elif ("bitmod" in wq_dtype.lower()):
            w, scales = search_group_dtype(w, wq_bits=n_bit)
    ...
    for i in tqdm(range(len(layers)), desc="pseudo weight quantization..."):
        ...
        for n, m in named_linears.items():
            ...
            m.weight.data = pseudo_quantize_tensor(
                m.weight.data, n_bit=w_bit, module_name=get_op_name(model, m), **q_config
            )
            m.cpu()
