...
def main():
    ...
    # ============================ data type and search method ============================
    # added for bitmod
    parser.add_argument('--datatype', type=str, default='int', choices=['int', 'fp', 'mod', 'arb', 'modext']) # fp and mod only supports bit width of 3 or 4
    parser.add_argument('--search_method', type=str, default='exha', choices=['exha', 'diff']) # exhaustive search or differentiable search, only effective when datatype is 'mod'
    ...
    args.model_family = '_'.join(args.net.lower().split('-')[:-1])  # so it can differentiate between llama llama-2 llama-3 llama0-3.2, and other model with different versions
    lm = LMClass(args)
    lm.seqlen = 2048
    lm.model.eval()
